...
import daomanagerabi from "../../utils/daomanagerabi.json";
import {
  Progress,
  Box,
  Radio,
...
export default function ExistingTokenForm() {
  ...
  const createDAO = async () => {
    if (window.ethereum._state.accounts.length !== 0) {
      ...
      console.log(process.env.NEXT_PUBLIC_DISCORD_GUILD_ID, "guildId");
      ...
      const channelData = {
        ...
        permission_overwrites: [
          {
            id: process.env.NEXT_PUBLIC_DISCORD_GUILD_ID,
            type: 0,
            allow: "0",
            deny: "1024",
          },
          { id: userId, type: 1, allow: "1024", deny: "0" },
          {
      ...
      const resp = await createDiscordChannel(
  ...
  async function createDiscordChannel(guildId, channelData) {
    try {
      const response = await axios.post("/api/create-discord-channel", {
        guildId,
        channelData,
      });
      toast({
        title: "Discord Channel Created Successfully",
        description: `Approve transaction from wallet to create DAO`,
        status: "info",
        duration: 10000,
        isClosable: true,
      });
